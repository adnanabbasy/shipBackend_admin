...
const listMarine = () => {
    ...
    function getService () {
        ...
        axios.get("http://34.254.97.212:8080/api/podcast/all").then((res)=>{
                            ...
                            {list.length > 0 && list.map((row, i) => (
                                <tr key={i}>
                                    ...
                                    {row.media_file && (
                                        (() => {
                                            try {
                                                let mediaArray = JSON.parse(row.media_file);
                                                return <audio controls>
                                                    <source src={`http://34.254.97.212:8080/${mediaArray.path}`} type="audio/mpeg" />
                                                    {/* <source src={`http://localhost:8080/${mediaArray.path}`} type="audio/mpeg" /> */}
                                                    Your browser does not support the audio element.
                                                </audio>
                                            } catch (error) {
                                                console.error('Error parsing row.media_file:', error);
                                                return null;
                                            }
                                        })()
                                    )}
                                    ...
                                    <td>{row.status}</td>
                                    <td>
                                        <Link to={`/admin/podcast/update/${row.id}`}>
