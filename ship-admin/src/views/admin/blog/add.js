...
const AddMarine = () => {
    ...
    const [marineCategoryId, setMarineCategoryId] = useState('1');
    ...
    const submitMarine = async(event) => {
        ...
        formData.append("category_id", marineCategoryId);
        ...
        try {     
            const response = await axios({
                method: "post",
                url: "http://34.254.97.212:8080/api/blogs/add",
                data: formData,
    ...
    const handleCategory = (event) => {
        setMarineCategoryId(event["value"])
        console.log("category Id", marineCategoryId)
    }

    useEffect(() => {
        axios.get('http://34.254.97.212:8080/api/blogcategory/all').then((res) => {
  ...
  return (
    <Fragment>
        <Card>
            <CardHeader>
                <CardTitle tag='h4'>Add Blog</CardTitle>
            </CardHeader>
